...
async def get_simulation_status(simulation_service: SimulationService = Depends(get_simulation_service)):
    return await simulation_service.get_status()


@router.post("/reset", response_model=Dict[str, Any])
async def reset_simulation(simulation_service: SimulationService = Depends(get_simulation_service)):
