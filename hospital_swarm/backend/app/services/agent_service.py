...
class AgentService:
    ...
    async def get_agent_by_id(self, agent_id: str) -> Optional[Agent]:
        ...
        RETURN a
        """
        results = await self.neo4j_service.run_query(query, {"agent_id": agent_id})
        if not results:
            return None
        return Agent(**results[0]["a"])
    ...
    async def update_agent(self, agent_id: str, agent_in: AgentUpdate) -> Optional[Agent]:
        ...
        results = await self.neo4j_service.run_query(
            query, 
            {"agent_id": agent_id, "props": update_data}
        )
        return Agent(**results[0]["a"])
    ...
    async def decide_actions(self, agents: List[Agent], pheromones: List[Pheromone]) -> List[str]:
        """
        Decide actions for agents based on current pheromones.
    ...
    def _decide_agent_action(self, agent: Agent, pheromones: List[Pheromone]) -> Optional[str]:
        """
        Decide a single action for an agent based on pheromones.
        Returns action description or None if no action.
        """
        # Filter active pheromones (not expired)
        active_pheromones = [p for p in pheromones if p.expires_at > datetime.now()] if hasattr(p, 'expires_at') else pheromones
        
        # For simplicity, we'll assume all pheromones passed in are active
        # In a real implementation, we would check expiration
        
        if agent.role == "nurse":
            # Nurse responds to emergency pheromones by triaging patients
            emergency_pheromones = [p for p in active_pheromones if p.type == "EMERGENCY" and p.strength > 0.6]
            if emergency_pheromones:
                # Find the strongest emergency
            ...
            # Doctor responds to emergency pheromones by diagnosing
            ...
            # ICU agent prepares beds for high-severity emergencies
            ...
            # Lab agent schedules tests for emergency cases
            ...
            # Pharmacy agent prepares medications
            emergency_pheromones = [p for p in active_pheromones if p.type == "EMERGENCY" and p.strength > 0.7]
            if emergency_pheromones:
                strongest = max(emergency_pheromones, key=lambda p: p.strength)
            ...
            # Ambulance responds to emergencies
        ...
        # Default: no action if no matching pheromones
        return None
